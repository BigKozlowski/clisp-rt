    ...
    (do ((y 0 (1+ y))
      ...
      (do* ((x 0 (1+ x))
        ...
        (let ((color (trace-ray scene (make-instance 'clrt-ray:ray
          ...
          (setf (aref image-data y x 0) (min 255 (truncate (m* 255.0 (vec-x color))))
                (aref image-data y x 1) (min 255 (truncate (m* 255.0 (vec-y color))))
                (aref image-data y x 2) (min 255 (truncate (m* 255.0 (vec-z color))))))))
...
(defun trace-ray (scene ray &optional (lower-bound 0.0) shadow-feeler)
  (let ((closest-match (find-closest-intersection scene ray lower-bound shadow-feeler)))
    (if closest-match
            ...
            (dolist (light (slot-value scene 'lights) color)
              (let* ((aux-dir-to-light (m- (light-pos light) ip))
                ...
                (unless (find-closest-intersection scene
                                                   ...
                                                   1e-3
                                                   dist-to-light)
                  (let* ((inv-ray-direction (m* -1.0 (clrt-ray:ray-direction ray)))
        ...
        (make-vector 3 :data #(0.0 0.0 0.0)))))
